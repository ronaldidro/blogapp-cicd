...
const App = () => {
  ...
  useEffect(() => {
    blogService.getAll().then((blogs) => setBlogs(blogs.sort((firstBlog, secondBLog) => secondBLog.likes - firstBlog.likes)))
  }, [])
  ...
  return (
    <div>
      ...
      <div className="blogList">
        {blogs.map((blog) => <Blog key={blog.id} blog={blog} updateBlog={putBlog} removeBlog={deleteBlog} loggedUser={user.username} />)}
      </div>
    </div>
  )
}
